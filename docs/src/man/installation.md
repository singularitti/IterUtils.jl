...
[IterUtils](https://github.com/singularitti/IterUtils.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("IterUtils")
   ```

3. Run

   ```julia-repl
   julia> using IterUtils
   ```
...
version of IterUtils, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/IterUtils.jl"
```
...
our [GitHub repository](https://github.com/singularitti/IterUtils.jl)
for new releases.
Once we release a new version, you can update IterUtils by typing

```@repl
using Pkg
Pkg.update("IterUtils")
Pkg.gc()
```
