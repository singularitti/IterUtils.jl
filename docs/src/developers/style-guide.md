...
!!! info
    The style guide is always a work in progress, and not all IterUtils code
    follows the rules. When modifying IterUtils, please fix the style violations
...
IterUtils uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/IterUtils.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the IterUtils directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to IterUtils have
